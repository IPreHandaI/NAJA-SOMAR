<?php
include_once 'includes/_head.php';
?>

<header>
    <div class="colsheader">
        <div class="headercol headercol1">
            <div>Logo</div>
        </div>
        <div class="headercol headercol2">
            <div class="dropdown1">
            <div class="dropdown-header2">
                <a>Selecione</a>
            </div>
                <div class="dropdown-options2">
                    <div class="dropdown-option2 option1" value="opcao1">
                        <p id="p">Opção 1</p>    
                    </div>
                    <div class="dropdown-option2 option2" value="opcao2">
                        <p>Opção 2</p>
                    </div>
                    <div class="dropdown-option2 option3" value="opcao3">
                        <p>Opção 3</p>
                    </div>
                    <div class="dropdown-option2 option4" value="opcao4">
                        <p>Opção 4</p>
                    </div>
                </div>            
            </div>
        </div>
        <div class="headercol headercol3">
            <div class="dropdown2">
                <div class="dropdown-header2">Selecione</div>
                <div class="dropdown-options2">
                    <div class="dropdown-option2 option1" value="opcao1">Opção 1</div>
                    <div class="dropdown-option2 option2" value="opcao2">Opção 2</div>
                    <div class="dropdown-option2 option3" value="opcao3">Opção 3</div>
                    <div class="dropdown-option2 option4" value="opcao4">Opção 4</div>
                </div>
            </div>
        </div>
        <div class="headercol headercol4">
            <div class="dropdown3">
                <div class="dropdown-header2">Selecione</div>
                <div class="dropdown-options2">
                    <div class="dropdown-option2 option1" value="opcao1">Opção 1</div>
                    <div class="dropdown-option2 option2" value="opcao2">Opção 2</div>
                    <div class="dropdown-option2 option3" value="opcao3">Opção 3</div>
                    <div class="dropdown-option2 option4" value="opcao4">Opção 4</div>
                </div>
            </div>
        </div>
        <div class="headercol headercol5">
            <div class="dropdown4">
                <div class="dropdown-header2">Selecione</div>
                <div class="dropdown-options2">
                    <div class="dropdown-option2 option1" value="opcao1">Opção 1</div>
                    <div class="dropdown-option2 option2" value="opcao2">Opção 2</div>
                    <div class="dropdown-option2 option3" value="opcao3">Opção 3</div>
                    <div class="dropdown-option2 option4" value="opcao4">Opção 4</div>
                </div>
            </div>
        </div>
    </div>

</header>
<main>